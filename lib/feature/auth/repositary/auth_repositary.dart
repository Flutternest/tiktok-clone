...
final authRepositaryProvider = Provider<AuthRepositary>((ref) {
  return  FirebaseAuthRepositary(auth: ref.watch(firebaseAuthProvider), firestore: ref.watch(firestoreprovider));
});

abstract class AuthRepositary{
  Future<Either<Failure, UserCredential>> signUp({required String firstName, required String lastName, required String email, required String password});
